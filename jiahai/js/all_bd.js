...
$.fn.dtlbba = function() {
    var $box = $("#picLiBaxd"), $smbox = $("#picLiBaxd .main-box"), $ul = $("#picLiBaxd .main-box ul"), $li = $("#picLiBaxd .main-box ul li"), btn = '<a href="javascript:void(0)" class="btns btn-left"></a><a href="javascript:void(0)" class="btns btn-right"></a>', width = $li.outerWidth(), rwidth = width + 22, len = $li.length, index = 0, startnum = 4, page = 1, v_width = rwidth * startnum, page_count = Math.ceil(len / startnum), picTimer;
    $box.append(btn);
    $ul.width(len * rwidth);
    var $btns = $("#picLiBaxd a.btns"), $left = $("#picLiBaxd a.btn-left"), $right = $("#picLiBaxd a.btn-right");
    if (screen.width <= 1024) {
        $left.css({
            left:0
        });
...
$.fn.videoList = function(){
    var $_this = $(this),
        $_vbig = $('.voide_list' , $_this),
        $_vsml = $('.videoSmallBox' , $_this),
        $_vsul = $('ul' , $_vsml),
        $_vsli = $('li' , $_vsul),
        $_vsif = $('img' , $_vsul),
       s_width = $_vsli.innerWidth(),
          _num = 4,
          _len = $_vsli.length,
         index = 0,
         _btns = '<a href="javascript:;" class="btns prev"></a><a href="javascript:;" class="btns next"></a>',
         video = '';
        for(var i = 0 ; i < _len ; i++){
            video += '<dd><h3 class="video_tit snh">'+$_vsif.eq(i).attr("data-tit")+'</h3><div class="viBox"><embed src="'+$_vsif.eq(i).attr("data-src")+'" allowFullScreen="true" quality="high" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash" wmode="transparent"></embed></div></dd>';
        };
        $_vbig.html(video);
        var $_vodd = $('dd' , $_vbig);
            $_vodd.eq(0).show().siblings().hide();
        $_vsul.width(s_width * _len);
        $_vsli.click(function(){
            var tt = $_vsli.index(this);
            $_vodd.eq(tt).show().siblings().hide();
        });
        if(_len > _num){
            $_vsml.append(_btns);
            var $_btn = $('.btns' , $_vsml),
                $_pre = $('.prev' , $_vsml),
                $_nex = $('.next' , $_vsml);
            $_vsml.hover(function(){
                $_btn.css({opacity:0.5}).stop(true,false).fadeIn();
            },function(){
                $_btn.stop(true,false).fadeOut();
            });
            $_pre.click(function(){
                index -= 1;
                if(index == -1) {index = _len - _num;}
                    listplay(index);
                });
            $_nex.click(function(){
                index += 1;
                    if(index >= _len - _num + 1) {index = 0;}
                listplay(index);                
            });
            function listplay(index){
                if(!$_vsul.is(":animated")){
                    $_vsul.stop(true,false).animate({left: - index * s_width},300);
                }
            }
        };
}
