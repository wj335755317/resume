function goList(){
	$("body,html").animate({scrollTop:$("#tplBtn").offset().top},200);
}
$(function() {//nav
	...
	$hov.hover(function(){
		if(screen.width > 1024){
			...
			$div.css({left:pwth+10,top:-30});
		}else{
			$ajt.attr('class','wxjtb');	
			$div.css({right:0,top:30});
		}
		$div.show();	
	},function(){
		$div.hide();	
    ...
    $.fn.banner = function(options) {
        ...
        $(ops.t_id).hover(function() {
        	if(ops.autoPlay){
            	...
            	$(ops.t_id + " ul").stop(true,false).slideDown();
            };
        }, function() {
        	if(ops.autoPlay){
        		...
        		$(ops.t_id + " ul").stop(true,false).slideUp();
        	}
        }).trigger("mouseleave");
  ...
  $.fn.tplbqx = function(){
  	var $abtn = $('#tplBtn ul li .cbtn'),
	$boas = $('#picBox div.m-main-a');
 ...
 $.fn.dtlb = function(){ //多图轮播
	var  $boxb = $('#picLiBox'),
	   $smboxb = $('#picLiBox .main-box'),
		  $ulb = $('#picLiBox .main-box ul'),
		  $lib = $('#picLiBox .main-box ul li'),
		  btnb = '<a href="javascript:void(0)" class="btns btn-left"></a><a href="javascript:void(0)" class="btns btn-right"></a>',
	   widthb = $lib.outerWidth(),
	   rwidthb = widthb+22,
		  lenb = $lib.length,
		indexb = 0,
	 startnumb = 4,
	     pageb = 1,
	  v_widthb = rwidthb *startnumb,      
 page_countb = Math.ceil(lenb / startnumb),
	picTimerb;
	$boxb.append(btnb);
	$ulb.width(lenb*rwidthb);
	var $btnsb = $("#picLiBox a.btns"),
		$leftb = $('#picLiBox a.btn-left'),
	   $rightb = $('#picLiBox a.btn-right');
	 if(screen.width <= 1024){
		 $leftb.css({left:0});
		 $rightb.css({right:0});
	 }
	 if(lenb>startnumb){
		$leftb.click(function() {
			if(!$ulb.is(":animated")){
				if(pageb==1){
					$ulb.animate({left:'-='+v_widthb * (page_countb-1) },"slow");
					pageb=page_countb;
				}else{
					$ulb.animate({left:'+='+v_widthb },"slow");
					pageb--;
				}
			}
		...
		$rightb.click(function() {
			if(!$ulb.is(":animated")){
				if(pageb==page_countb){
					$ulb.animate({left:'0px'},"normal");
					pageb = 1;
				}else{
					$ulb.animate({left:'-='+v_widthb},"normal");
					pageb++;
				}
			}			
...
$.fn.dtlba = function(){ //多图轮播
	var  $box = $('#picLiBax'),
	   $smbox = $('#picLiBax .main-box'),
		  $ul = $('#picLiBax .main-box ul'),
		  $li = $('#picLiBax .main-box ul li'),
		  btn = '<a href="javascript:void(0)" class="btns btn-left"></a><a href="javascript:void(0)" class="btns btn-right"></a>',
	    width = $li.outerWidth(),
	   rwidth = width+22,
		  len = $li.length,
		index = 0,
	 startnum = 4,
	     page = 1,
	  v_width = rwidth *startnum,      
 page_count = Math.ceil(len / startnum),
	picTimer;
	$box.append(btn);
	$ul.width(len*rwidth);
	var $btns = $("#picLiBax a.btns"),
		$left = $('#picLiBax a.btn-left'),
	   $right = $('#picLiBax a.btn-right');
	 if(screen.width <= 1024){
		 $left.css({left:0});
		 $right.css({right:0});
	 }
	 if(len>startnum){
		...
		$right.click(function() {
			if(!$ul.is(":animated")){
				if(page==page_count){
					$ul.animate({left:'0px'},"normal");
					page = 1;
				}else{
					$ul.animate({left:'-='+v_width},"normal");
					page++;
				}
			}			
...
$.fn.jdhd = function(){ //jdcxhdm
	var $btn  = $('#jdbox ul.topbtns li'),
		$list = $('#jdbox ul.toplist li');
...
$.fn.teams = function(options) { //teams
	...
	for (var i = 1; i <= s; i++) {
		...
		span.eq(6 * i - 1).find(".ewm-s").css({right:0,left:0});
	}
	for (var i = 0; i < len; i++) {
...
},$.fn.teamb = function(options) { //teams
	...
	for (var i = 1; i <= s; i++) {
		...
		span.eq(4 * i - 1).find(".ewm-s").css({right:0,left:0});
	}
	for (var i = 0; i < len; i++) {
		span.eq(i).hover(function() {
			$(this).children(".ewm-s").show();
